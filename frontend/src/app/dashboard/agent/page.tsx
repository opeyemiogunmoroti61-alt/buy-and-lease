...
import {
  Loader, Plus, Home, Eye, Edit2, CheckCircle,
  Clock, TrendingUp, Users, Building2, Star,
  PhoneCall, Mail, ArrowRight
} from "lucide-react";
      ...
      {results.length > 0 && (
        <ul className="border rounded-md shadow-sm bg-white max-h-52 overflow-y-auto z-10 relative">
          {results.map((r) => (
            <li
              key={r.place_id}
              className="px-3 py-2 text-sm hover:bg-violet-50 cursor-pointer border-b last:border-0"
              onClick={() => {
    ...
    <ProtectedRouteWithRole>
      <div className="min-h-screen bg-slate-50 pt-24 pb-16 px-4">
        <div className="max-w-6xl mx-auto space-y-6">
          ...
          {/* ── Header ──────────────────────────────────────────────────── */}
          <div className="bg-gradient-to-r from-violet-600 via-purple-600 to-indigo-600 rounded-2xl p-8 text-white">
            <div className="flex items-center justify-between flex-wrap gap-4">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <Star className="w-5 h-5 text-yellow-300 fill-yellow-300" />
                  <span className="text-violet-200 text-sm font-medium">Licensed Agent</span>
                </div>
                <h1 className="text-3xl font-black mb-1">
                  Welcome, {user?.username || "Agent"} 👔
                </h1>
                <p className="text-violet-200">
                  Manage your property portfolio and client listings
                </p>
              </div>
              <Button
                onClick={() => setView(view === "add" ? "overview" : "add")}
                className="bg-white text-violet-700 hover:bg-violet-50 font-semibold flex items-center gap-2"
              >
                {view === "add" ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { icon: Building2, label: "Total Listings", value: listings.length, color: "bg-violet-50 text-violet-600" },
              ...
              { icon: TrendingUp, label: "Performance", value: "—", color: "bg-indigo-50 text-indigo-600" },
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
            ...
            <div className="grid grid-cols-2 gap-3 mt-5 pt-5 border-t border-slate-100">
              ...
              <div className="flex items-center gap-2 text-sm text-slate-400">
                ...
                <span className="italic">Phone not set</span>
              </div>
            </div>
          </div>
          ...
          {view === "add" && (
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-8">
              <h2 className="text-xl font-bold text-slate-900 mb-2">List a New Property</h2>
              <p className="text-slate-500 text-sm mb-6">
                Search and select the property address to get started
              </p>
              <div className="max-w-lg space-y-4">
                ...
                >
                  {loader ? <Loader className="animate-spin" /> : "Next — Add Property Details"}
                </Button>
              </div>
            </div>
          )}
          ...
          {view === "overview" && (
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
              <div className="flex items-center justify-between mb-6">
                ...
                <Link href="/properties" className="text-sm text-violet-600 hover:underline flex items-center gap-1">
              ...
              {loadingListings ? (
                ...
                <div className="text-center py-16">
                  ...
                  <p className="text-slate-400 text-sm mb-4">
                    Start listing properties on behalf of your clients
                  </p>
                  <Button
                    onClick={() => setView("add")}
                  ...
                  {listings.map((listing) => (
                    ...
                    >
                      <div className="relative h-44 bg-slate-100">
                        ...
                        {listing.type && (
                          <span className="absolute top-2 right-2 text-xs font-bold px-2.5 py-1 rounded-full text-white bg-violet-600">
                      ...
                      <div className="p-4 space-y-2">
                        ...
                        <div className="flex gap-2 pt-1">
                          ...
                          <Link
                            ...
                            className="flex-1 flex items-center justify-center gap-1.5 text-xs bg-violet-600 text-white rounded-xl py-2 hover:bg-violet-700 transition-colors font-medium"
                          >
